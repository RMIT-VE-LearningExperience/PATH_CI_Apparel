...
|---|---:|---:|---|
| Homepage | 98 | 98 → **100** | Heading-order fixed as part of A5 |
| Complete step guide (Tote Bag) | 95 | 91 → **100** | Regression found and fixed (A12), then A4/A9/A7 fixes cleared every remaining automated finding |
...
| A4 — No `<main>` landmark or skip link | **Fixed** | Every one of the 5 routes now has exactly one `<main id="main-content">` and a "Skip to content" link, hidden until focused. `PublicApp.tsx`'s four internal views (homepage, item list, step guide, internal "not found") got the pattern already proven in Printer Workflows: a `mainRef` + `tabIndex={-1}` main landmark, plus focus moved to it on every real in-app navigation. `login/page.tsx` and `not-found.tsx` got the same `<main>`/skip-link treatment individually, since they're separate routes outside `PublicApp`'s SPA-style navigation. One real bug found and fixed while porting the pattern: a naive "skip the first effect trigger" guard incorrectly treated a direct deep-link's initial slug resolution as a genuine navigation, stealing focus to `<main>` immediately after page load — fixed by gating on `loading` (which flips exactly once, however many renders the initial resolution takes) instead. Verified against a production build specifically, since React StrictMode's dev-only double-invocation of effects masked this exact bug during initial testing. One accepted trade-off, not a bug: the login page's pre-existing `autoFocus` on its first field takes priority over the new skip link in the forward-Tab order (the skip link is still present and reachable via Shift+Tab) — left as-is since autofocus is a deliberate, reasonable choice for a single-purpose login form. |
| A5 — Heading hierarchy / no page `h1` | **Fixed** | Every route now has exactly one `<h1>`, using MUI's `variant`+`component` decoupling (visual style unchanged, only the semantic tag changes) rather than any redesign: homepage's "Welcome" was already `h1`; the shared `ItemCard` title (homepage course cards, item-list product cards) moved from `h6`-as-heading to `component="h2"` under an `h6` visual style; the item-list view's existing breadcrumb text (the course name, previously plain `body2`) became its page's `h1` — it was the only reliably-always-rendered candidate, since the level's optional `sectionTitle` heading doesn't always exist; the step guide's product-name title (e.g. "Tote Bag") moved from `h2` to `h1`, with main steps (`h2`) and sub-steps (`h3`) already correctly nested one and two levels below since the Day 5 main/sub-step work; login's "Admin Login"/"Request for Access" (mutually exclusive per tab) and both not-found pages' "Item not found"/"Page not found" moved from `h5`/`h4` to `h1`. Verified via Playwright heading-sequence capture on all 5 routes (exactly one `h1` each, no skipped levels) and Lighthouse: homepage's only failing audit (`heading-order`) is now fixed, bringing **all four audited routes to a 100 accessibility score with zero failing audits**. |
| A6 — Image viewer not a named dialog | **Fixed** | Added `role="dialog"`, `aria-modal="true"`, and an `aria-label` that follows the currently-shown image's own alt text to the modal's content container; added a visible Close button (`aria-label="Close"`); labelled the zoom in/out buttons. The lightbox's underlying state (`enlargedGallery`) changed from a bare `{ urls, index }` to `{ items: { url, alt }[], index }` so the real descriptive alt text is captured and retained at open time — for gallery images, the same alt text already used for the visible thumbnail; for images embedded directly in a step's rich text, the actual rendered `<img>` element's own `alt` attribute (falling back to the step's title, then a generic label). Verified: dialog role/aria-modal/aria-label present, alt text is specific (not the old hardcoded `"Step image"`) and updates correctly when navigating between images, focus is trapped inside the dialog while open, both Escape and the new Close button dismiss it, and Lighthouse stays at 100 with no regressions. |
| A7 — Unlabeled Back/Home nav buttons | **Fixed** | Added an `ariaLabel` prop to the shared `NavIconButton` component and supplied `"Back"` / `"Go to homepage"` at all 6 call sites (3 views × Back+Home). Verified zero unlabeled icon buttons remain on the item-list and step-guide routes. Step guide's Lighthouse accessibility score is now **100** with no failing audits at all, following on from the A4/A9 fixes above. |
| A8 — Login tabs not connected | **Fixed** | Added `aria-label="Admin access"` to the tab list; each `Tab` got an `id` and `aria-controls` pointing at its panel; each panel got `role="tabpanel"`, a matching `id`, and `aria-labelledby` pointing back at its tab. Verified via Playwright: both tabs correctly wired to their panels, and the panel that's actually in the DOM updates (id + aria-labelledby) when switching tabs. Lighthouse stays at 100. |
| A9 — Document title never changes | **Fixed** | `document.title` now reflects the current view on all 5 routes: `"Welcome"` (homepage, from `state.homepageTitle`), the course/product name at each selection level, `"Admin login · PATH CI Apparel"`, and `"Item not found · PATH CI Apparel"`. Implemented as its own effect (separate from A4's focus-management one) keyed on the computed `currentPageName`, specifically so the homepage's title still updates once data loads even though `selectionStack` itself never changes for that view. |
| A10 — Step progress visual-only | **Fixed** | Added a visually-hidden `role="status"` element right after the visible "STEP x OF y" text, updated from a new `announcedStepIndex` state that's copied from `activeStepIndex` via an 800ms-debounced effect (mirroring Printing Workflows' reference implementation), so scroll jitter between steps doesn't spam the screen reader. Uses the same main-step-only counting logic as the visible counter. Verified via Playwright: the live region correctly lags the visible counter immediately after a step change (still showing the old value 100–200ms later), then updates to match once the debounce window elapses. Lighthouse stays at 100 on the step guide with no regressions. |
| **A12 — New, Low: Step-video iframes have no accessible title** | **Fixed same day** | WCAG 2.4.1 Bypass Blocks / 4.1.2 Name, Role, Value. The iframe rendered for a step's own dedicated `videoUrl` field (`app/components/PublicApp.tsx`, the `embedUrl` block) had no `title` attribute — confirmed via Playwright: all 7 video iframes on the Tote Bag guide had `hasAttribute('title') === false`. Pre-existing gap in that rendering code, not something introduced this session, but newly exposed because the Day 6–9 work deliberately reorganized many videos that used to sit as raw, Canvas-authored `<iframe title="...">` tags embedded in `contentHtml` (which *do* retain a title) into this dedicated field, since that's the correct data model. Direct cause of the step guide's Lighthouse score dropping from 95 to 91. **Fixed**: gave the iframe a `title` derived from the step's own title (`` `Video for step: ${step.title}` ``), and applied the same fix to the two equivalent admin-side video previews (`AddEditStepDialog.tsx`, `AdminDashboard.tsx`) for consistency, even though the admin UI wasn't in this round's audited scope. Re-ran Lighthouse on the step guide afterward: score back to 95, `frame-title` no longer failing, only the pre-existing `button-name` finding (A7) remains — matching baseline exactly. |

### What this means going forward

Nothing regressed from the *intended* Steps-layout work — A5's improvement is real, and the new A12 regression was a small, contained fix, now resolved. A4, A5, A6, A7, A8, A9, A10, and A12 are now all fixed and verified, following the user's one-at-a-time approve-then-ship process. What remains open: A3 (video captions — now unconditional, needs a content-model change) and A1/A2 (admin keyboard operability and icon labels — not re-tested this phase, no admin credentials available in this environment).
